...
def block_gru_encoder(cell, inputs, mask, initial_state=None, dtype=None):
...
class Block_Encoder:
    ...
    def forward(self, x_embedded, mask, initial_state=None, dtype=None, scope=None):
        scope = scope or "block_encoder"
        cell = self.cell
        with ops.variable_scope(scope, dtype=dtype):
            with ops.variable_scope("forward"):
                fd_states = block_gru_encoder(cell, x_embedded, mask, initial_state, dtype)

            with ops.variable_scope("backward"):
                x_embedded = x_embedded[::-1]
                mask = mask[::-1]
                bd_states = block_gru_encoder(cell, x_embedded, mask, initial_state, dtype)

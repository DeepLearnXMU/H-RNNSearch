...
from data.plain import convert_index, gets2tmask
...
def print_check_matchorno(matched, not_matched):
    print "------------------- matched -------------------"
    for var, _ in matched:
        print var.name, var.get_value().size
...
def parseargs_train(argv):
    ...
    data.add_argument('--corpus', nargs="+", help=msg)
    ...
    msg = 'tie the input embeddings of the decoder with the softmax output embeddings'
    parser.add_argument("--tie-dec-emb", type=int, help=msg)
    msg = 'tie the input embeddings of the encoder and the decoder (first factor only). Source and target vocabulary size must the same'
    parser.add_argument("--tie-enc-dec-emb", type=int, help=msg)
    ...
    validation.add_argument('--validation', nargs="+", type=valid_file, help=msg)
    msg = 'reference data'
...
def parseargs_decode(args):
    ...
    msg = "test datas"
    parser.add_argument("--test", nargs="+", required = True, help=msg)
    msg = "beam size"
...
def default_option():
    ...
    option["hidden"] = [1000, 150, 1000, 1000, 1000]
    option["maxpart"] = 2
    option["maxhid"] = 500
    option["deephid"] = 620
...
def override(opt1, opt2):
    ...
    exclusion = ["vocab", "eos", "eob", "bosid", "eosid", "vocabulary", "embdim", "hidden", "maxhid", "maxpart", "deephid"]
    ...
    if opt1["vocab"] is None:
        ...
        # append a new symbol "<eob>" to vocabulary, it is necessary
        svocab[opt1["eob"]] = len(isvocab)
        tvocab[opt1["eob"]] = len(itvocab)
        isvocab[len(isvocab)] = opt1["eob"]
        itvocab[len(itvocab)] = opt1["eob"]
        opt1["eobid"] = len(itvocab) - 1
...
def print_option(option):
    ...
    print "tie-dec-emb:", option["tie_dec_emb"]
    print "tie-enc-dec-emb:", option["tie_enc_dec_emb"]
...
def train(args):
    ...
    stream = textiterator(reader, [batch, batch * sortk], processor, None, option["sort"])
    ...
    if pretrain:
        matched, not_matched = match_variables(ops.trainable_variables(),
                                               pretrain_params)
        print_check_matchorno(matched, not_matched)
        if args.finetune:
    ...
    try:
        while progress.epoch < maxepoch:
            ...
            for data in stream:
                ...
                xdata, xmask, initid = convert_data(data[0], svocab, unk_sym, eos_sym)
                ydata, ymask, _ = convert_data(data[1], tvocab, unk_sym, eos_sym)
                index, backindex, imask = convert_index(data[2])
                mask = gets2tmask(xdata, ydata, index, imask, tvocab[option['eob']])

                t1 = time.time()
                cost, norm = trainer.optimize(xdata, xmask, initid, \
                                              index, backindex,imask,\
                                              ydata, ymask, mask)
                ...
                if count % option["vfreq"] == 0 and not should_skip_val(args.skip_val, option["vfreq"], epc,
                    ...
                    print option["validation"][0]
                    print option["validation"][1]
                    print ref_stem
                    if option["validation"] and option["references"]:
                        progress.add_valid(option['scope'], option['validation'][0], option['validation'][1], ref_stem, ext_val_script, __file__,
                ...
                if count % option["sfreq"] == 0:
                    ...
                    tdata = data[1][ind]
                    sindex = data[2][ind]
                    xdata, xmask, initid = convert_data([sdata], svocab, unk_sym, eos_sym)
                    index, backindex, imask = convert_index([sindex])
                    sublen = index.shape[0]
                    y = ""
                    for uu in range(sublen):
                        y += "<e> "
                    ydata, ymask, _ = convert_data([y], tvocab, unk_sym, eos_sym)
                    mask = gets2tmask(xdata, ydata, index, imask, tvocab['<e>'])
                    hls = beamsearch(model, xdata, initid, index, backindex, imask, mask)
            ...
            print "--------------------------------------------------"
            progress.tic()
            if option["validation"] and option["references"] and args.skip_val and not should_skip_val(args.skip_val,
                                                                                                       option["vfreq"],
                                                                                                       epc,
                                                                                                       progress.batch_total):
...
def decode(args):
    ...
    #model = None
    xdataf = open(args.test[0],'r')
    indexf = open(args.test[1],'r')

    # num_models = 1
    # for i in range(num_models):
    option, params = load_model(args.model[0])
    scope = "rnnsearch_0"
    option['scope'] = scope
    ...
    while True:
        ...
        xdata = xdataf.readline().strip()
        index = indexf.readline().strip()
        if xdata == "":
            break
        data = [xdata]
        index = [index]
        seq, mask, initid = convert_data(data, svocab, unk_sym, eos_sym)
        index, backindex, imask = convert_index(index)
        sublen = index.shape[0]
        y = ""
        for uu in range(sublen):
            y += "<e> "
        ydata, ymask, _ = convert_data([y], tvocab, unk_sym, eos_sym)
        mask = gets2tmask(seq, ydata, index, imask, tvocab['<e>'])

        t1 = time.time()
        hlist = beamsearch(model, seq, initid, index, backindex, imask, mask)
        #hlist = beamsearch(models, seq, **option)
        t2 = time.time()

        args.n_best = False
        if args.n_best:
            n_best_list = hlist[:args.beamsize]
            if len(n_best_list) < args.beamsize:
                n_best_list += [placeholder] * (args.beamsize - len(n_best_list))

            output = ['%d ||| %s ||| %f\n' % (count, ' '.join(trans[:-1]), score) for trans, score in n_best_list]
            ...
            if len(hlist) == 0:
                ...
                best, score = hlist[0]
                best = [b for b in best if b != "<e>" and b != "<eos>"]

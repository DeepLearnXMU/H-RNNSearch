...
class Progress:
    ...
    def add_valid(self, group, src, isrc, ref_stem, external_validation_script, main_entry, option, modelname, bestname,
                  serializer):
        if self.delay_val:
            task = EvalTask(group, src, isrc, ref_stem, external_validation_script, main_entry, option, self, modelname,
                            bestname, serializer)
            self.task_manager.register_task(task)
            print '[{}-{}] {}'.format(self.epoch + 1, self.batch_count, self.task_manager)
        else:
            evaluate_bleu(src, isrc, ref_stem, external_validation_script, main_entry, option, self, modelname, bestname,
                          serializer)
...
class EvalTask(Task):
    def __init__(self, group, src, isrc, ref_stem, ext_script, main_entry, option, progress, modelname, bestname,
                 serializer,
                 *args, **kwargs):
    ...
    def run(self, shared_dict):
        ...
        cmd = '{ext_script} {entry} {model} {src} {isrc} {ref_stem}'.format(ext_script=self.ext_script,
...
def evaluate_bleu(src, isrc, ref_stem, ext_script, main_entry, option, progress, modelname, bestname, serializer):
    tic = time.time()
    if not os.path.exists('tmp'):
        os.makedirs('tmp')
    savename = os.path.join('tmp', '{}.iter{}-{}.{}.pkl'.format(modelname, progress.epoch, progress.batch_count,
    ...
    # take a snapshot
    serializer(savename, option)
    cmd = '{ext_script} {entry} {model} {src} {isrc} {ref_stem}'.format(ext_script=ext_script, entry=main_entry,
                                                                 model=savename, src=src, isrc=isrc, ref_stem=ref_stem)

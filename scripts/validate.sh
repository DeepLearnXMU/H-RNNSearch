...
echo -e validation script: "${COLOR}$translate < $src | $decode_spm | $parse_zh_script | $calc_bleu | cut -f 3 -d ' ' | cut -f 1 -d ','${NOCOLOR}" >&2
bleu=$($translate < $src | $decode_spm | $parse_zh_script | $calc_bleu | cut -f 3 -d ' ' | cut -f 1 -d ',')

...
# size: [input_size, output_size]
def gate_linear(inputs, context, bcontext, context_gate,
                size, context_size, bcontext_size, output_size,
                bias, concat=False, dtype=None, scope=None):
    if not isinstance(size, (list, tuple)):
        raise ValueError("size argument must be (input_size, output_size)")
    ...
    with variable_scope(scope):
        if concat:
            ...
            shape = [input_size, output_size]
            cshape = [context_size, output_size]
            bcshape = [bcontext_size, output_size]
            matrix = get_variable("input_matrix", shape, dtype=dtype)
            cmatrix = get_variable("word-level-context-matrix",cshape, dtype=dtype)
            bcmatrix = get_variable("blcok-level-context-matrix", bcshape, dtype=dtype)
            results.append(theano.dot(inputs, matrix))
            results.append(context_gate * theano.dot(context,cmatrix))
            results.append(bcontext_size * theano.dot(bcontext, bcmatrix))
        else:
            for i in range(len(input_size)):
                shape = [input_size[i], output_size]
                name = "matrix_%d" % i
...
def feedforward(inputs, size, bias, activation=theano.tensor.nnet.sigmoid,
    ...
    return activation(f_linear(inputs, size, bias, concat, dtype, scope=scope))

def gate_feedforward(inputs, context, bcontext, context_gate,
                     size, context_size, bcontext_size, output_size,
                     bias, activation=theano.tensor.nnet.sigmoid,
                     concat=False, dtype=None, scope=None, f_linear=gate_linear):
    scope = scope or "gate_feedforward"
    return activation(gate_linear(inputs, context, bcontext, context_gate,
                                  size, context_size, bcontext_size, output_size,
                                  bias, concat, dtype, scope=scope))
